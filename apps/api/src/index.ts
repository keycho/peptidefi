...
// ─── Public-launch hardening middleware ────────────────────────────
//
// All four pieces below are mounted at startup in buildApp().
// They're kept here (rather than in a separate lib file) because
// they read closure variables (startedAt, version) and because the
// public-API contract documented in docs/PUBLIC_API.md is owned by
// this file.
...
function isAdminBypass(req: express.Request): boolean {
  ...
  // Trim because the express dashboard sometimes appends "\n" to env
...
function buildApp(): express.Express {
  ...
  //     tight bound the public launch pins. Pre-launch this was
  //     30/min — see commit history for the rationale on the change.
...
// Only run main() when this module is invoked as the process entry
// point — not when imported by tests. ESM equivalent of CJS
// `require.main === module`: compare the module's own URL against
// the URL form of process.argv[1]. Under `pnpm start` tsx invokes
// this file directly and the two match; under `vitest` the test
// file is the entry point and they don't.
const isEntryPoint = (() => {
  try {
