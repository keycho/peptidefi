        ...
        const builder: Record<string, unknown> = {
          ...
          then(
            ...
            return Promise.resolve(
              globalThis.__researchResolver!(table),
            ).then(onF);
          },
        };
        return builder;
...
describe("research handler — input validation", () => {
  ...
  it("uppercases :code before lookup (case-insensitive entry)", async () => {
    ...
    setResolver((table) => {
      ...
      if (table === "peptide_research_metadata")
        return { data: META_ROW, error: null };
      if (table === "supplier_observations")
        return { data: [OBS_ROW], error: null };
      if (table === "commit_cycles")
        return { data: [CYCLE_ROW], error: null };
      if (table === "twap_commits")
        return { data: [LATEST_TWAP_ROW], error: null };
      return { data: null, error: null };
    });
    const { res, status, body } = makeRes();
    await getResearchHandler(makeReq("bpc157"), res);
    expect(status()).toBeUndefined(); // 200 default (we don't call .status)
    expect((body() as { peptide: { code: string } }).peptide.code).toBe(
      "BPC157",
    );
  });
});
...
describe("research handler — happy path response shape", () => {
  function fullResolver(table: string): { data: unknown; error: unknown } {
    ...
    if (table === "peptide_research_metadata")
      return { data: META_ROW, error: null };
    if (table === "twap_commits")
      return { data: [HISTORY_ROW], error: null };
  ...
  it("returns 200 with the documented shape for BPC157", async () => {
    ...
    expect(b.peptide.aliases).toEqual([
