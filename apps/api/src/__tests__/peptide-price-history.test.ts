      ...
      from(table: string) {
        const builder: Record<string, unknown> = {
          ...
          then(onF: (v: { data: unknown; error: unknown }) => unknown) {
            return Promise.resolve(globalThis.__priceHistoryResolver!(table)).then(onF);
          },
        };
        return builder;
      },
...
declare global {
  ...
  var __priceHistoryResolver: ((table: string) => { data: unknown; error: unknown }) | undefined;
}
...
describe('price-history handler — input validation', () => {
  it('400 when :code violates the [A-Z0-9]{2,16} pattern', async () => {
    const { getPeptidePriceHistoryHandler } = await import('../routes/v1/peptide-price-history');
  ...
  it('400 when days exceeds the max of 90', async () => {
    ...
    await getPeptidePriceHistoryHandler(makeReq('BPC157', { days: '365' }), res);
    expect(status()).toBe(400);
  });
});
...
describe('price-history handler — 404 contracts', () => {
  ...
  it('404 when ?vendor= names an unknown supplier code', async () => {
    ...
    const { res, status, body } = makeRes();
    await getPeptidePriceHistoryHandler(makeReq('BPC157', { vendor: 'DOESNOTEXIST' }), res);
    expect(status()).toBe(404);
...
describe('price-history handler — happy path', () => {
  it('200 + documented shape for BPC157 with defaults', async () => {
    ...
    setResolver((table) => {
      ...
      if (table === 'twap_commits')
        return {
          data: [twap({ value: '6.699', hoursAgo: 1 }), twap({ value: '6.700', hoursAgo: 24 })],
          error: null,
        };
      return { data: null, error: null };
    });
  ...
  it('?days=30 reflected in the window length', async () => {
    ...
    await getPeptidePriceHistoryHandler(makeReq('BPC157', { days: '30' }), res);
  ...
  it('?vendor=PUREHEALTH narrows to one series', async () => {
    ...
    const { res, body } = makeRes();
    await getPeptidePriceHistoryHandler(makeReq('BPC157', { vendor: 'PUREHEALTH' }), res);
    const b = body() as {
...
describe('price-history handler — pure helpers', () => {
  it("truncateToBucket('daily') zeros hours, minutes, seconds", async () => {
    ...
    const out = _internal.truncateToBucket('2026-05-13T14:37:22.123Z', 'daily');
    expect(out).toBe('2026-05-13T00:00:00.000Z');
  });

  it("truncateToBucket('hourly') zeros minutes + seconds, keeps the hour", async () => {
    const { _internal } = await import('../routes/v1/peptide-price-history');
    const out = _internal.truncateToBucket('2026-05-13T14:37:22.123Z', 'hourly');
...
/* ─── IPFS CID surfacing on twap_series ────────────────────────────
 *
 * The price-history endpoint aggregates twap_commits rows into JS-side
 * buckets (daily/hourly). Each output bucket carries one ipfs_cid —
 * the CID from the freshest row inside the bucket — so consumers
 * have an audit anchor without us inflating the response into a
 * per-row array.
 */
describe('price-history handler — ipfs_cid on twap_series', () => {
  it('surfaces the most-recent CID inside each bucket and tolerates null', async () => {
    const { getPeptidePriceHistoryHandler } = await import('../routes/v1/peptide-price-history');
    setResolver((table) => {
      if (table === 'peptides') return { data: PEPTIDE_BPC157, error: null };
      if (table === 'supplier_observations') return { data: [], error: null };
      if (table === 'twap_commits')
        return {
          data: [
            // Two rows landing in the same daily bucket (~24h apart but
            // for default 14-day window with daily bucketing, the
            // freshest CID wins per UTC-day).
            twap({ value: '6.6', hoursAgo: 4, ipfsCid: 'bafyolder' }),
            twap({ value: '6.7', hoursAgo: 2, ipfsCid: 'bafyfresher' }),
            // A bucket with no CID (pin not yet succeeded).
            twap({ value: '6.5', hoursAgo: 26, ipfsCid: null }),
          ],
          error: null,
        };
      return { data: null, error: null };
    ...
    const { res, body } = makeRes();
    await getPeptidePriceHistoryHandler(makeReq('BPC157', { aggregation: 'hourly' }), res);
    const b = body() as {
      twap_series: Array<{
        timestamp: string;
        twap_value_usd_per_mg: number;
    ...
    // Every output point exposes the ipfs_cid field.
    for (const p of b.twap_series) {
      expect(Object.prototype.hasOwnProperty.call(p, 'ipfs_cid')).toBe(true);
    }
    // The 26h-ago row sits in its own hourly bucket with cid=null.
    const nullBucket = b.twap_series.find((p) => p.ipfs_cid === null);
    expect(nullBucket).toBeTruthy();
    // The two recent rows sit in different hourly buckets; both carry
    // their own CID.
    const cids = b.twap_series.map((p) => p.ipfs_cid).filter((c) => c !== null);
    expect(cids).toEqual(expect.arrayContaining(['bafyolder', 'bafyfresher']));
  });

  it('picks the latest-computed_at CID within a single daily bucket', async () => {
    const { getPeptidePriceHistoryHandler } = await import('../routes/v1/peptide-price-history');
    setResolver((table) => {
      if (table === 'peptides') return { data: PEPTIDE_BPC157, error: null };
      if (table === 'supplier_observations') return { data: [], error: null };
      if (table === 'twap_commits')
        return {
          data: [
            twap({ value: '6.6', hoursAgo: 5, ipfsCid: 'bafyolder' }),
            twap({ value: '6.7', hoursAgo: 1, ipfsCid: 'bafyfresher' }),
          ],
          error: null,
        };
      return { data: null, error: null };
    ...
    const { res, body } = makeRes();
    await getPeptidePriceHistoryHandler(makeReq('BPC157', { aggregation: 'daily' }), res);
    const b = body() as {
      twap_series: Array<{ ipfs_cid: string | null; cycle_count: number }>;
    };
    expect(b.twap_series).toHaveLength(1);
    expect(b.twap_series[0]!.cycle_count).toBe(2);
    expect(b.twap_series[0]!.ipfs_cid).toBe('bafyfresher');
  });
});
